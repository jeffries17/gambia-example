...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run island-based restaurant analysis')
    
    # Use new directory structure
    parent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    default_output_dir = os.path.join(parent_dir, "outputs", "by_sector", "restaurants")
    ...
    print(f"Running island-based restaurant analysis...")
    ...
    try:
        ...
        # Create and run the restaurant analyzer with island segmentation
        restaurant_analyzer = RestaurantAnalyzer(
            sentiment_analyzer, 
            output_dir=args.output_dir
        )
        island_results = restaurant_analyzer.run_island_analysis(df=enriched_df)
        
        print(f"Island-based restaurant analysis complete! Results saved to: {args.output_dir}")
    
    except Exception as e:
        print(f"Error during restaurant analysis: {str(e)}")
        import traceback
        traceback.print_exc()
